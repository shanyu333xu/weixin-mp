      ...
      success: (response) => {
        ...
        stockEntries.forEach((entry) => {
          ...
          const stockData: OriStockData = {
            ...
            amount: parseFloat((parseFloat(elements[9]) / 100000000).toFixed(2)),
        ...
        const stockDataList: StockData[] = oriStockDataList.map((element) => {
          const change = parseFloat((element.currentPrice - element.closePrice).toFixed(2))
          const changePercent = parseFloat(((change / element.closePrice) * 100).toFixed(2))
          const speedPercent = parseFloat(
            (((element.currentPrice - element.openPrice) / element.closePrice) * 100).toFixed(2),
          )
          return {
            ...element,
            change,
